...
* `Interp_Correct`: interpolates $\frac{1}{z}$ and $\frac{\Phi}{z}$, where $\Phi$ is the vertex attribute and $z$ is the vertex depth. The final fragment attribute is then

$$\frac{\verb|interpolate|(\frac{\Phi}{z})}{\verb|interpolate|(\frac{1}{z})}$$

Note that the code uses `inv_w` (see `ClippedVertex` definition) to represent $\frac{1}{z}$, so you do not need to directly access $z$ at the vertices.
