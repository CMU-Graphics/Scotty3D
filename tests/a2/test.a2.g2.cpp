...
static void expect_linear(Halfedge_Mesh& mesh) {
...
Test test_a2_g2_linear_square("a2.g2.linear.square", []() {
	Halfedge_Mesh sqr({{1, 3, 2, 0}},
	                  {Vec3{-0.5f, 0.0f, -0.5f}, Vec3{-0.5f, 0.0f, 0.5f},
	                   Vec3{0.5f, 0.0f, -0.5f}, Vec3{0.5f, 0.0f, 0.5f}});

	Halfedge_Mesh sqr_linear({{3, 0, 1, 2}, {5, 0, 3, 4}, {7, 0, 5, 6}, {1, 0, 7, 8}},
...
Test test_s2_g2_linear_quad_cube("a2.g2.linear.quad_cube", []() {
	...
	Halfedge_Mesh quad_cube_linear(
	...
	expect_linear(quad_cube);

	if (auto diff = Test::differs(quad_cube, quad_cube_linear)) {
		throw Test::error("Result does not match expected: " + diff.value());
	}
});
